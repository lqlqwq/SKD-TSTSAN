...
def create_loso_structure():
    """
    为SMIC数据集创建LOSO结构并添加光流
    """
    # 源文件夹和目标文件夹
    source_folder = '../../Dataset/SMIC_retinaface_classify'
    target_folder = '../../Dataset/SMIC_retinaface_classify_LOSO'
    ...
    for subject in subjects:
        ...
        for emotion_class in emotion_classes:
            ...
            subject_files = [file for file in os.listdir(emotion_path) if file.startswith(subject + "_")]
            other_files = [file for file in os.listdir(emotion_path) if not file.startswith(subject + "_")]
    ...
    for emotion_class in os.listdir(classify_path):
        ...
        for img in os.listdir(emotion_path):
            if '_onset.jpg' in img:  # 只处理onset图片
                base_name = img.replace('_onset.jpg', '')  # 例如：s1_ne_01
